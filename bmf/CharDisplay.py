...
class CharDisplay(QtGui.QWidget):
  """
  ...
  def paintEvent(self,event):
     ...
     xmag=self.width()/self.columns
     ymag=self.height()/self.rows
     sz=10
  ...
  def __init__(self,log,rows,columns,parent=None):
     """
     Initialize the character display of columns width  x rows height.
     """
     super(CharDisplay, self).__init__(parent)
